...
RSpec.describe Organized::TimeOffPeriodPolicy do
  let(:user) { create :user }
  let(:organization_member) { create :organization_member, user: user }
  subject { described_class }

  permissions :create? do
    let(:resource) { create :time_off_period, organization_id: organization_member.organization.id }
    ...
    it 'grants access if user is the period author' do
  ...
  permissions :update?, :destroy? do
    ...
    it 'denies access if resource is not in the same organization' do
      ...
      resource = create :time_off_period, organization_id: organization.id
      organization_member.update_column :admin, true
      expect(subject).not_to permit(organization_member, resource)
    end
